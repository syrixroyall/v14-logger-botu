...
client.on("ready", async () => {
  console.log(`${client.user.tag} | By Syrix`);
  client.user.setPresence({ activities: [{ name: config.presence ? config.presence : `By Syrix`, type: ActivityType.Streaming, url: "https://www.twitch.tv/directory" }], status: "dnd" });
})
...
async function logG(beşG) {
  let kanalG = client.channels.cache.get(config.genellog)
  if(!kanalG)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalG.send(beşG)
}
async function logK(beşK) {
  let kanalK = client.channels.cache.get(config.kanallog)
  if(!kanalK)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalK.send(beşK)
}
async function logR(beşR) {
  let kanalR = client.channels.cache.get(config.rollog)
  if(!kanalR)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalR.send(beşR)
}
async function logE(beşE) {
  let kanalE = client.channels.cache.get(config.emojilog)
  if(!kanalE)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalE.send(beşE)
}
async function logN(beşN) {
  let kanalN = client.channels.cache.get(config.userlog)
  if(!kanalN)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalN.send(beşN)
}
async function logS(beşS) {
  let kanalS = client.channels.cache.get(config.sunuculog)
  if(!kanalS)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalS.send(beşS)
}
async function logM(beşM) {
  let kanalM = client.channels.cache.get(config.mesajlog)
  if(!kanalM)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalM.send(beşM)
}
async function logV(beşV) {
  let kanalV = client.channels.cache.get(config.voicelog)
  if(!kanalV)return console.log("Log Kanalı Bulunamadı | Kanal ID'sini Kontrol Ediniz!")
  kanalV.send(beşV)
}

//bu altyapı WDeW-e Aittir!